...
<div class="pageContent">
	<form id="order_edit" method="post" action="<%=basePath %>/order/root/add" class="pageForm required-validate" onsubmit="return iframeCallback( this, dialogAjaxDone )">
		<input type="hidden" name="infoUrl.parentId" id="parentId" value="${infoUrl.parentId}" />
		<div class="pageFormContent" layoutH="85">
			<table style="padding:20px 0 0 30px;width:100%;">				
				<tr class="unit unit_tr">
					<th width="100%;" colspan="2" style="background-color:#009933;">${infoUrl.parentId!=-1?"添加子节点信息":"添加跟节点信息"}</th>				
				</tr>
				<c:if test="${infoUrl.parentId!=-1}">
					<tr class="unit unit_tr">
						<td width="10%;">父级菜单：</td>
						<td width="80%;">
				...
				<tr class="unit unit_tr">
					<td>url地址：</td>
					<td>
						<input type="text" name="infoUrl.href" style="width:300px;" maxlength="120" class="string" value="${infoUrl.href}" onblur="setRel(this.value)" />
				...
				<tr class="unit unit_tr">
					<td>目标对象target：</td>
					<td>
						<input type="text" name="infoUrl.target" style="width:300px;" maxlength="20" class="string" value="${infoUrl.target==null?'navTab':infoUrl.target==''?'navTab':''}" />
				...
				<tr class="unit unit_tr">
					<td>rel：</td>
					<td>
						<input type="text" name="infoUrl.rel" id="rel" style="width:400px;" maxlength="120" class="string" value="${infoUrl.rel}" />
